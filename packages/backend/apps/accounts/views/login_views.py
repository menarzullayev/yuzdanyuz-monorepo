...
class OTPVerifyHTMXView(View):
    ...
    def post(self, request):
        ...
        try:
            ...
            return render(
                request,
                'accounts/partials/phone_step2.html',
                {
                    'phone': raw_phone,
                    'error': str(e),
                },
...
class PhoneRegisterFormView(View):
    ...
    def get(self, request):
        return render(request, 'accounts/partials/phone_step1.html', {'mode': 'register'})
...
class EmailLoginView(View):
    ...
    def post(self, request):
        ...
        if '@' in login_val:
            email_addr = EmailAddress.objects.filter(user=user, email__iexact=login_val).first()

            if email_addr and not email_addr.verified:
                return HttpResponse(
                    'Email tasdiqlanmagan. Tekshirish havolasini emailda tekshiring.', status=403
                )
        ...
        try:
            if reg_method == 'email':
                ...
                if not username or not re.match(r'^[a-zA-Z0-9_]{3,}$', username):
                    return HttpResponse(
                        'Foydalanuvchi nomi 3+ ta belgi, harflar/raqamlar/pastki chiziq', status=400
                    )
