...
from .models import CustomUser, Region, District


@admin.register(CustomUser)
class CustomUserAdmin(UserAdmin):
    model       = CustomUser
    list_display  = ['username', 'display_name', 'phone_number', 'telegram_id', 'user_type', 'is_staff']
    list_filter   = ['is_staff', 'is_superuser', 'is_active', 'preferred_lang', 'study_year', 'region']
    search_fields = ['username', 'first_name', 'last_name', 'phone_number', 'email', 'telegram_username']
    ...
    fieldsets = UserAdmin.fieldsets + (
        ('Telegram', {'fields': (
            ...
            'telegram_language', 'tg_linked_at',
        )}),
    ...
    add_fieldsets = UserAdmin.add_fieldsets + (
        ('Kontakt', {'fields': ('phone_number', 'email')}),
    )
