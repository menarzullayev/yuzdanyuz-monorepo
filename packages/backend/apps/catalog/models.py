...
class Topic(models.Model):
    ...
    parent = models.ForeignKey(
        ...
        related_name='children', verbose_name=_("Yuqori mavzu")
    )
...
class Question(TenantTimestampMixin):
    ...
    discrimination_index = models.FloatField(default=1.0, help_text=_("IRT Discrimination Index (a)"))

    # Ko'p tilli bog'lash
    language = models.CharField(max_length=10, default='uz')
    parent_question = models.ForeignKey(
        'self', on_delete=models.SET_NULL, null=True, blank=True,
        related_name='translations', help_text=_("Asosiy tildagi savol bilan bog'lash")
    )
...
class QuestionVersion(models.Model):
    ...
    adaptive_feedback = models.JSONField(null=True, blank=True, verbose_name=_("Moslashuvchan qayta aloqa"))
    metadata         = models.JSONField(default=dict, blank=True, help_text=_("Vaqt limiti va boshqa qoidalar"))
...
class AIProviderConfig(models.Model):
    ...
    name     = models.CharField(max_length=100, verbose_name=_("Nom"), help_text=_("Masalan: 'Asosiy Claude', 'Zaxira Ollama'"))
    provider = models.CharField(max_length=20, choices=Provider.choices, verbose_name=_("Provayder"))
    model    = models.CharField(max_length=100, verbose_name=_("Model nomi"),
                                help_text=_("claude-sonnet-4-6 | gpt-4o | gemini-1.5-pro | llama3"))
    api_key  = models.CharField(max_length=500, blank=True, verbose_name=_("API kalit"),
                                help_text=_("Ollama uchun bo'sh qoldiring"))
    base_url = models.CharField(max_length=500, blank=True, verbose_name=_("Base URL"),
                                help_text=_("Faqat Ollama uchun: http://localhost:11434"))

    use_for   = models.CharField(max_length=20, choices=UseFor.choices,
                                 default=UseFor.ALL, verbose_name=_("Foydalanish maqsadi"))
    is_active = models.BooleanField(default=True, verbose_name=_("Faol"))
    priority  = models.PositiveSmallIntegerField(default=1, verbose_name=_("Ustuvorlik"),
    ...
    def __str__(self):
        ...
        return f"{status} [{self.priority}] {self.name} ({self.get_provider_display()}: {self.model})"
