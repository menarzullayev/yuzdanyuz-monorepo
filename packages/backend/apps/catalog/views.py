...
from .models import ImportBatch, QuestionDraft, Question, QuestionVersion, Subject, Topic
...
def _get_batch(batch_id, request):
    ...
    qs = ImportBatch.global_objects if hasattr(ImportBatch, 'global_objects') else ImportBatch.objects
...
def review_dashboard(request, batch_id):
    ...
    return render(request, 'catalog/review_dashboard.html', {
        'batch':    batch,
        'counts':   counts,
        'subjects': subjects,
    })
...
def draft_detail_htmx(request, draft_id):
    ...
    return render(request, 'catalog/partials/draft_detail.html', {
        'draft':    draft,
        'subjects': subjects,
        'topics':   topics,
    })
...
def _toast(message: str, kind: str = "info", reload: bool = False) -> HttpResponse:
    ...
    oob  = ' hx-swap-oob="true"' if True else ''
    reload_trigger = (
