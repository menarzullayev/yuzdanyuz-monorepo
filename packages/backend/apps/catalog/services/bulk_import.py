...
from apps.catalog.models import (
    Question, QuestionVersion, QuestionDraft, ImportBatch,
    Subject, Topic
)
            ...
            for row_number, row in enumerate(self.rows, 2):  # 2-chi qatordan boshlash (header skip)
                ...
                try:
                    subject = Subject.objects.get(
                        name__iexact=row.get('subject', '').strip(),
                        organization=self.organization
                    )
                    if row.get('topic'):
                        topic = Topic.objects.filter(
                            name__iexact=row.get('topic', '').strip(),
                            subject=subject
                        ).first()
...
def bulk_import_from_file(file, file_type: str, organization: Organization, batch: ImportBatch) -> Dict[str, Any]:
